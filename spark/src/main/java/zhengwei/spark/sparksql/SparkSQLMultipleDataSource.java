...
import zhengwei.spark.udf.LongDateToStringDate;
...
//		result.write().text("e:/temp/spark/sql/text");
    ...
    @Test
    void read() {
        final Dataset<Row> load = spark
                .read()
                .format("csv")
                .option("delimiter", "\t")
                .option("inferSchema", "true")
                .load("D:\\gdga\\part-00000");
        load.printSchema();
        String[] strings = new String[]{"_c1"};
        String s = "_c0";
        Column[] columns = new Column[3];
        columns[0] = load.col("_c0");
        columns[1] = load.col("_c1");
        columns[2] = load.col("_c2");
        load.select(columns).toDF("a","b","c").show();
    }
    ...
    void testUdf() {
        final Dataset<Row> load = spark
                ...
                .option("delimiter", "\t")
                .option("inferSchema", "true")
                .load("C:\\Users\\zw305\\Desktop\\part-00000");
        String[] strings = new String[]{"_c2"};
        Column[] columns = new Column[1];
        columns[0] = load.col("_c2");
        final Dataset<Row> timeTable = load.select(columns).toDF("time");
        timeTable.registerTempTable("timeTable");
        spark.udf().register("test", new LongDateToStringDate(), DataTypes.StringType);
        spark.sql("select count(1) from timeTable group by test(time,'HHmm')").show();
    }
}
